...
import { Briefcase, RefreshCw, ArrowDownLeft, ArrowUpRight, Percent, FileText, List } from 'lucide-react';
...
export const OperationsPage: React.FC = () => {
  ...
  const fetchOperations = async () => {
    ...
    try {
      ...
      const to = new Date();
      const from = new Date();
      from.setFullYear(to.getFullYear() - 10); // Last 10 years to mimic "All time"
  ...
  const getOperationLabel = (type: string) => {
    const key = `operations.types.${type}`;
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-slate-900/50 text-slate-400 text-sm border-b border-slate-700">
                <th className="p-4 font-medium">{t('operations.date')}</th>
                <th className="p-4 font-medium">{t('operations.type')}</th>
                <th className="p-4 font-medium">{t('operations.asset')}</th>
                <th className="p-4 font-medium">{t('operations.account')}</th>
                <th className="p-4 font-medium text-right">{t('operations.amount')}</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-700/50">
              {isLoading && operations.length === 0 ? (
                ...
                <tr>
                   <td colSpan={5} className="p-8 text-center text-slate-500">{t('operations.noOperations')}</td>
                </tr>
              ) : (
                operations.map((op) => {
